import { ArrowDown, ArrowUpRight, Bookmark, Compass } from 'lucide-react';
  ...
  return (
    <section className="landing-shell pt-10 sm:pt-16">
      {/* <div className="flex items-center justify-between border-y editorial-rule py-3 editorial-mono text-[10px] uppercase tracking-[.12em]" style={{ color: 'var(--muted)' }}>
        ...
        <span className="hidden sm:inline">Independent ideas · intentionally assembled</span>
        <span>New Delhi · 27°C</span>
      </div> */}
      <div className="grid gap-10 py-12 lg:grid-cols-[1.1fr_.9fr] lg:items-start lg:py-24">
        <div>
          <p className="editorial-label reveal mb-5">Your personal daily edition</p>
          <h1 className="editorial-serif reveal delay-1 max-w-4xl text-[clamp(3rem,6.5vw,6rem)] font-black leading-[.9] tracking-[-.06em]">News with<br /><em className="font-semibold" style={{ color: 'var(--accent)' }}>more</em> than one<br />side of the story.</h1>
        ...
        <div className="relative mx-auto w-full max-w-[530px] lg:mt-6">
          <div className="floating-note absolute -left-12 -top-8 z-10 hidden max-w-[145px] border border-[var(--rule)] bg-[var(--paper)] p-3 shadow-lg sm:block"><div className="editorial-label text-[8px]">Different angle</div><p className="mt-2 editorial-serif text-sm leading-4">One story. Three lenses. Your view gets wider.</p></div>
          ...
          <div className="absolute -bottom-7 right-1 z-10 flex items-center gap-2 border border-[var(--rule)] bg-[var(--paper)] px-3 py-2 shadow-md sm:right-8"><span className="editorial-mono text-[9px] uppercase tracking-wider">Curated for your curiosity</span></div>
        </div>
      </div>
    </section>
  );
